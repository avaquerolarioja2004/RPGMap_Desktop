...
namespace RPGMap
{
    public partial class ContentObject : Form
    {
        ...
        public ContentObject(Generar generar, Button bttAddobject)
        {
            InitializeComponent();
            this.bttAddobject = bttAddobject;
            this.generar = generar;
            listaobject = CRDObject.GetAllObjects().Where(objeto => !generar.ListaObject.Contains(objeto)).ToList();
        }

        private void bttGen_Click(object sender, EventArgs e)
        {
            if (generar.ListaObject.Count > 0)
            {
